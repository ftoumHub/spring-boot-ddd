...
class PaymentProcessManagerImpl implements PaymentProcessManager {
    ...
    public PaymentProcessManagerImpl(ApplicationContext applicationContext) {
    ...
    @Override
    public CompletionStage<Either<CommandFailure, Tuple2<PaymentId, PaymentStatus>>> process(PaymentCommand.PerformPayment performPayment) {
    ...
    public CompletionStage<Either<CommandFailure, Tuple2<PaymentId, PaymentStatus>>> fallback(PaymentCommand.PerformPayment performPayment) {
    ...
    private CompletionStage<Either<CommandFailure, Tuple2<PaymentId, PaymentStatus>>> confirm(Payment payment, PaymentRequested acceptPayment, PaymentAuthorized acceptAuthorization) {
        PaymentCommand.ConfirmPayment confirmPayment = PaymentCommand.ConfirmPayment.commandOf(
    ...
    private CompletionStage<Either<CommandFailure, PaymentAuthorized>> authorize(Payment payment, PaymentRequested acceptPayment) {
        PaymentCommand.AuthorizePayment authorizePayment = PaymentCommand.AuthorizePayment.commandOf(
